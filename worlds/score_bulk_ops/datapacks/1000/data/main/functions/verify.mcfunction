...
execute unless score m c matches 999 run scoreboard players add ng c 1
execute unless score m c matches 999 run say [score_bulk_ops] verify: NG (agg_max が 999 を返していない)
...
execute unless score r999 _ matches 999 run scoreboard players add ng c 1
execute unless score r999 _ matches 999 run say [score_bulk_ops] verify: NG (往復後に r999 が 999 に戻っていない)

# 3. gc_core を通す
function main:gc_core
execute unless score r0 _ matches 0 run scoreboard players add ng c 1
execute unless score r999 _ matches 999 run scoreboard players add ng c 1
execute unless score m c matches 998 run scoreboard players add ng c 1
execute unless score m c matches 998 run say [score_bulk_ops] verify: NG (gc_core 中の max が 998 になっていない)
